...
document
  ...
  .addEventListener("click", async () => {
    ...
    const username = usernameField.querySelector("input").value.trim() || null;
    const password = passwordField.querySelector("input").value.trim() || null;
    ...
    try {
      const res = await fetch("/web/database/create", {
        ...
        body: JSON.stringify({ name, type, username, password, port, ipv4 }),
      });
